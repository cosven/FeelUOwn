...
- 缺陷修复
  ...
  - 修复“红心雷达”的切换，以及一些问题 #985

5.0a1 (2026-01-19)
""""""""""""""""""""""
- 功能增强

  - 调整 AI 相关功能，引入多个 agent（Copilot agent 和推荐 agent） #969
  - 随机播放的时候，不会重复播放，也支持播放上一首 #980
  - 内置“红心雷达”的功能，资源方实现相应接口接口 #983
  - 使用 pyproject.toml 来管理项目元信息及其依赖 #974
  - 支持在命令行触发视频播放 #970

- 缺陷修复

  - 修复专辑列表有时候显示不全的问题 #976
  - 修复展示专辑/歌单/视频卡片时，有一定概率崩溃的问题 #982

- 兼容性变更

  - 引入 pyproject.toml 来管理项目（理论上不影响上下游兼容性，建议下游逐步适配） #974
  - 抛弃 py3.8/3.9 支持，只支持 3.10 及其以上版本 #970
  - 修改“双击播放歌单歌曲”的行为：默认会播放整个歌单 #977
...
- 新特性
  ...
  - 优化新用户体验 #965
    ...
    - 应用启动后默认进入主页
    - 优化登录框的 UI，添加登录提示
    - 支持添加歌手/专辑/歌单到音乐库 #964

  - 优化专辑/歌单/视频卡片的展示 #965

- 功能增强

  - 使用 PyQt6 替换 PyQt5 #954 #956
  - 优化淡入淡出曲线 #960 #958
